...
export async function handleChat(request, clientRawRequest = null) {
    ...
    if (subUserContext) {
      for (const comboModel of comboModels) {
        ...
        if (!comboModelInfo?.provider || !isProviderAllowedForSubUser(subUserContext, comboModelInfo.provider)) {
          return errorResponse(HTTP_STATUS.FORBIDDEN, "This combo includes providers that are not enabled for the current sub-user");
        }
      }
    }
  ...
  return handleSingleModelChat(body, modelStr, clientRawRequest, request, apiKey, subUserContext);
}

/**
 * Handle single model chat request
 */
async function handleSingleModelChat(body, modelStr, clientRawRequest = null, request = null, apiKey = null, subUserContext = null) {
  ...
  if (!isProviderAllowedForSubUser(subUserContext, provider)) {
    log.warn("AUTH", `Sub-user attempted to access disabled provider: ${provider}`);
