...
🎉 Bảng điều khiển mở tại `http://localhost:20502`
...
URL mặc định:
- Bảng điều khiển: `http://localhost:20502/dashboard`
- API tương thích OpenAI: `http://localhost:20502/v1`
...
Settings → Models → Advanced:
  OpenAI API Base URL: http://localhost:20502/v1
...
```json
{
  "anthropic_api_base": "http://localhost:20502/v1",
...
```bash
export OPENAI_BASE_URL="http://localhost:20502"
...
export NEXT_PUBLIC_BASE_URL="http://localhost:20502"
...
| `PORT` | framework default | Cổng dịch vụ (`20502` trong các ví dụ) |
| `HOSTNAME` | framework default | Bind host (Docker mặc định là `0.0.0.0`) |
| `NODE_ENV` | runtime default | Đặt `production` để triển khai |
| `BASE_URL` | `http://localhost:20502` | URL cơ sở nội bộ phía máy chủ được sử dụng bởi các tác vụ đồng bộ đám mây |
...
- Đặt `PORT=20502` và `NEXT_PUBLIC_BASE_URL=http://localhost:20502`

**Lỗi đồng bộ đám mây**
- Xác minh `BASE_URL` trỏ đến phiên bản đang chạy của bạn (ví dụ: `http://localhost:20502`)
...
```bash
POST http://localhost:20502/v1/chat/completions
...
```bash
GET http://localhost:20502/v1/models
Authorization: Bearer your-api-key
...
- cục bộ (`http://127.0.0.1:20502/v1/chat/completions`): hoạt động với `stream=false` và `stream=true`.
