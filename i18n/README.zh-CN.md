...
默认 URL：
- 仪表板：`http://localhost:20502/dashboard`
- OpenAI 兼容 API：`http://localhost:20502/v1`
...
```json
{
  ...
  "models": {
    "providers": {
      "9router": {
        "baseUrl": "http://127.0.0.1:20502/v1",
        "apiKey": "sk_9router",
...
export NEXT_PUBLIC_BASE_URLhttp://localhost:20502"
...
| `PORT` | 框架默认值 | 服务端口（示例中为 `20502`） |
| `HOSTNAME` | 框架默认值 | 绑定主机（Docker 默认为 `0.0.0.0`） |
| `NODE_ENV` | 运行时默认值 | 部署时设置 `production` |
| `BASE_URL` |http://localhost:20502` | 云同步作业使用的服务器端内部基础 URL |
...
**仪表板在错误的端口打开**
- 设置 `PORT=20502` 和 `NEXT_PUBLIC_BASE_URL=http://localhost:20502`

**云端同步错误**
- 验证 `BASE_URL` 指向您正在运行的实例（例如：`http://localhost:20502`）
...
```bash
POST httplocalhost:20502/v1/chat/completions
...
- 本地运行时（`http://127.0.0.1:20502/v1/chat/completions`）：使用 `stream=false` 和 `stream=true` 都可以工作。
